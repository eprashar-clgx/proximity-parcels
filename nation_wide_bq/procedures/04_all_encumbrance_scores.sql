...
CREATE OR REPLACE PROCEDURE `clgx-gis-app-dev-06e3.proximity_parcels.consolidate_all_scores`()
OPTIONS(strict_mode=false) -- To suppress errors while uploading procedure through console
BEGIN

  CREATE OR REPLACE TABLE `clgx-gis-app-dev-06e3.proximity_parcels.all_encumbrance_scores`
  ...
  FROM `clgx-gis-app-dev-06e3.proximity_parcels.parcels_mv` AS p

  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.proximity_intersection_railways` p1
    ON p.parcelPTID = p1.parcelPTID

  -- FIX: Cast the ID column to STRING to match the type of p1.encumbrance_id
  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.railways_mv` l1
    ON p1.encumbrance_id = CAST(l1.FRAARCID AS STRING)

  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.proximity_intersection_roadways` p2
    ON p.parcelPTID = p2.parcelPTID
  -- FIX: Cast the ID column to STRING to match the type of p2.encumbrance_id
  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.roadways_mv` l2
    ON p2.encumbrance_id = CAST(l2.ID AS STRING)

  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.proximity_intersection_transmission_lines` p3
    ON p.parcelPTID = p3.parcelPTID
  -- FIX: Cast the ID column to STRING to match the type of p3.encumbrance_id
  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.transmission_lines_mv` l3
    ON p3.encumbrance_id = CAST(l3.ID AS STRING)

  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.proximity_intersection_protected_lands_national` p4
    ON p.parcelPTID = p4.parcelPTID
  -- This join is likely okay since UUIDs are strings, but casting is a safe practice.
  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.protected_lands_national_mv` l4
    ON p4.encumbrance_id = CAST(l4.ID AS STRING)

  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.proximity_intersection_wetlands` p5
    ON p.parcelPTID = p5.parcelPTID
  -- FIX: Cast the ID column to STRING to match the type of p5.encumbrance_id
  LEFT JOIN `clgx-gis-app-dev-06e3.proximity_parcels.wetlands_mv` l5
